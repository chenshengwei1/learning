...
window.refreshHistory = function(){
	let aArraysStr = localStorage.getItem('history');
	if (!aArraysStr){
		aArraysStr = '[]';
	}
	let aArrays = JSON.parse(aArraysStr);
	let str = '';
	for(let item of aArrays){
		str+=`<li title="${item}"><div class="remove-history" onclick="removeWithHistory(event)">Delete</div><a href="javascript:void" onclick="openWithHistory(event)">${item}</a></li>`
	}
	$('.history ul').attr('length', aArrays.length);
	$('.history ul').html(str);
}

setTimeout(()=>{window.refreshHistory();})
...
function handleEvent() {
	...
	window.openWithHistory = function(event){
		let url = $(event.target).text();
		openWith(url);
	}
	window.removeWithHistory = function(event){
		let url = $(event.target).closest('li').attr('title');
		var r = confirm("You will delete [" + url+'], are you sure?');
		if (r==true)
		{
			deleteHistory(url);
		}
	...
	window.openWith = function(url){
		
		$('.current-play-url').html(url);
		dp1.switchVideo({
            url: url,
	...
	window.addHistory = function(url){
        ...
        let aArrays = JSON.parse(aArraysStr);
        if(aArrays.indexOf(url) == -1){
            aArrays.push(url);
            localStorage.setItem('history', JSON.stringify(aArrays));
			refreshHistory();
        }
	}
	
	window.reload = function(){
		let url = $('#urlinnput').val();
		addHistory(url);
		openWith(url);
	}
	
	window.deleteHistory = function(url){
		let aArraysStr = localStorage.getItem('history');
        if (!aArraysStr){
            aArraysStr = '[]';
        }
        let aArrays = JSON.parse(aArraysStr);
		let index = aArrays.indexOf(url);
        if(index !== -1){
            aArrays.splice(index,1)
            localStorage.setItem('history', JSON.stringify(aArrays));
			refreshHistory();
        }
	}
	...
	window.playVideo = function(){
		let date = $('#urlinnput1').val();
		
		let num = $('#urlinnput2').val();
		let url = `https://cl-sp.clsp-001.com/${date.replaceAll('-','')}/${num||0}/index.m3u8`;
        openWith(url);
	}
	
	window.nextPlayVideo = function(){
		let num = Number($('#urlinnput2').val())+1;
		$('#urlinnput2').val(num);
		playVideo();
	}
	
	window.prepagePlayVideo = function(){
		let num = Number($('#urlinnput2').val())-1;
		$('#urlinnput2').val(num);
		playVideo();
	}
	window.date2string = function(d){
		let y = d.getFullYear();
		let m = d.getMonth()+1;
		m = m>9?m:'0'+m;
		let da = d.getDate();
	...
	window.preDayPlayVideo  = function(){
		let date = $('#urlinnput1').val();
		let d1 = new Date(date);
		let d = new Date(d1.getTime()-(24*60*60*1000));
		$('#urlinnput1').val(date2string(d));
		playVideo();
	}
	window.nextDayPlayVideo  = function(){
		let date = $('#urlinnput1').val();
		let d1 = new Date(date);
		let d = new Date(d1.getTime()+(24*60*60*1000));
		$('#urlinnput1').val(date2string(d));
...
function initPlayers() {
    ...
    window.dpFloat = new DPlayer({
        ...
        screenshot: true,
        video: {
            url: 'https://cl-sp.clsp-001.com/20220527/33/index.m3u8',
...
{ 
	...
	if(origin.match(/hyaa[\d]+\.com/)){
		window.addHistory(event.data.url);
	}

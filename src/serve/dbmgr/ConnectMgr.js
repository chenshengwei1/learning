...
var createSchema = function(schemaName, model){

    if (!schemaName || !schemaName.test(/[a-zA-Z][\d\w_]{0, 31}/)){
    ...
    // 定义模式
    const Schema = mongoose.Schema;
    const SomeModelSchema = new Schema(model);

    // 使用模式“编译”模型
    const SomeModel = mongoose.model(schemaName, SomeModelSchema);
    return SomeModel;
}
